...
def test_the_prompt_scales_the_threshold_by_volatility_in_both_directions():
    """The batch died at the gate when this guidance was a bare number.

    Measured live: given only "a falsifier near 6% trips about half the time,
    choose per name based on how volatile it is", the model read 6% as a floor
    and adjusted upward only. Every defensive name came back at 6-8% against a
    mid-band threshold of 2.8-3.6%, scored near 0.20, and five of six were
    rejected as too hard before any mechanism was read. The prompt has to carry
    the scale and the direction, not just the reference point.
    """
    snap = make_snapshot()
    client, parse = fake_client(ThesisBatch(theses=[draft()]))
    LLMResearch(client=client, horizon_days=60).propose(snap, 3)
    ...
    assert "not a floor" in prompt
    assert "Adjust in both directions." in prompt
    # The universe's mean volatility is what the reference threshold belongs to,
    # so it has to be in the prompt for the ratio to mean anything.
    mean_vol = LLMResearch(client=client)._mean_vol(snap)
    assert f"{mean_vol:.0%}" in prompt
    assert "ann.vol / " in prompt, "the scaling rule itself, not just an exhortation"


def test_mean_volatility_is_the_universe_average_of_the_table_column():
    """The prompt quotes one number and the table quotes each name's; a drift
    between them would make the ratio the model is told to compute wrong."""
    snap = make_snapshot()
    source = LLMResearch(client=None)
    per_name = [source._ann_vol(snap.histories[s]) for s in snap.universe]
    assert source._mean_vol(snap) == pytest.approx(float(np.mean(per_name)))
