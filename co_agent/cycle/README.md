...
- **`LLMResearch` has never executed.** It needs `ANTHROPIC_API_KEY`; the tests
  run against an injected client. At ~$0.13 per cycle it is effectively free —
  the earlier $5–25 estimate was for a multi-turn loop, which this is not.
